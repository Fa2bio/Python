...
if colunas_a == linhas_b:
    ...
    for i in range(len(matriz_c)):
        for j in range(len(matriz_c)):
            if i == j:
                if matriz_c[i] == 1 and matriz_c[j] == 1:
                    identi = True
                else:
                    identi = False

    if identi == True:
        print('A multiplicação destas matrizes gerou a matriz identidade')
    else:
        print('A multiplicação destas matrizes não gerou a matriz identidade')
